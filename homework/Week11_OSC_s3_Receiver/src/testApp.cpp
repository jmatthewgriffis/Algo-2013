...
void testApp::setup(){
    ...
    ofSetWindowPosition( ( ofGetScreenWidth() - ( ofGetWidth() * 2.0 ) ) / 2.0, ( ofGetScreenHeight() - ( ofGetHeight() * 2.0 ) ) / 2.0 + ofGetHeight() ); // All four windows will be centered together, this one in the lower-left.
    
    // This one listens to the same port that we were sending to in the other app.
    mReceiver.setup( 54321 );
